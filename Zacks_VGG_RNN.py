...
if not os.path.exists(modelDir):
    os.mkdir(modelDir)
modelDir = join(modelDir, 'RNN')
if not os.path.exists(modelDir):
    os.mkdir(modelDir)
modelDir = addDateTime(join(modelDir, 'trial'))
if not os.path.exists(modelDir):
    os.mkdir(modelDir)
modelPath = join(modelDir, 'model_')
activeLearningInput = "1"  # 1: True; 0: False
...
def loadData(jsonData, inPath):
    ...
    for vid in jsonData.keys():
        ...
        batchPaths = batchPaths + sorted(
            [str(join(vidPath, f) + '/') for f in listdir(vidPath) if isdir(join(vidPath, f))])
    return batchPaths
...
with tf.Session() as sess:
    ...
    numTrainParams = np.sum([np.prod(v.get_shape().as_list()) for v in tf.trainable_variables()])
    print('Begin Training... Number of Trainable Parameters: {}'.format(numTrainParams))
    ...
    for i in range(n_epochs):
        ...
        for miniBatchPath in tqdm(batch):
            ...
            vidName, minibatches = loadFurnitureMiniBatch(miniBatchPath, mode='RNN')
            segCount = 0
            ...
            for x_train in tqdm(minibatches):
                ...
                ret = sess.run([train_op, sseLoss, sseLoss1, curr_state1, fc1],
                               feed_dict={inputs: x_train, is_training: True, init_state1: new_state,
                                          learning_rate: lr})
                new_state = ret[3]
            ...
            print('Average Minibatch Prediction Error: {}'.format(np.mean(predError)))
